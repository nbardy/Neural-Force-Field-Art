...
1. Fuse `directionOrderLoss` (§7 of collapse note: moments + dL/dF =
   (dL/du − u(u·dL/du))/s, parity fixture incl. all-zero-field guard) so
   pressure can default ON for adversary pieces without dropping back to
   tfjs. Until then pressure is opt-in via URL.
2. Consider baking user's 2026-08-17 hand-tuned settings (above) into a new
   gallery piece once they can export/share them.
3. Soak flake attribution (see fusion note) — hashgrid vs pair4 ×5 runs.
