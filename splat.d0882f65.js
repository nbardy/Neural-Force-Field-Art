...
let f=document.getElementById("splat"),g=document.getElementById("prompt"),m=document.getElementById("optimize"),h=document.getElementById("nudge"),v=document.getElementById("reset"),$=document.getElementById("readout"),w=document.getElementById("notice");function b(e){w.textContent=e}function x(e){p.error=e,p.phase="error",b(e),$.textContent="—",// eslint-disable-next-line no-console
...
let S=Function("u","return import(u)"),j=null,E=null;async function q(){if(E)return;let e=await S("https://cdn.jsdelivr.net/npm/@huggingface/transformers@4.2.0/+esm");e.env.allowRemoteModels=!0;let r="Xenova/mobileclip_s0";j=await e.AutoTokenizer.from_pretrained(r),E=await e.CLIPTextModelWithProjection.from_pretrained(r,{dtype:"fp32",device:"wasm"})}async function T(e){await q();let r=await j(e,{padding:"max_length",max_length:77,truncation:!0}),t=await E(r),i=t.text_embeds.data,a=new Float32Array(512);for(let e=0;e<512;e++)a[e]=i[e];return a}// ── Optimize loop ─────────────────────────────────────────────────────────────
...
u.step(),u.step(),R+=2,p.step=u.stepCount,R>=14&&(R=0,A())),function(){if(!_)return;let e=i.createCommandEncoder(),r=e.beginRenderPass({colorAttachments:[{view:a.getCurrentTexture().createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:1}}]});r.setPipeline(n),r.setBindGroup(0,_),r.draw(3),r.end(),i.queue.submit([e.finish()])}(),requestAnimationFrame(W)}async function M(){if(!p.ready)return;let e=g.value.trim()||"a photo of a cat";m.disabled=!0,p.phase="encoding",p.running=!1,b("encoding prompt (first use downloads the text model — slow)…"),y();try{let r=await T(e);G=r,u.setPrompt(r);// Baseline cos on the CURRENT splats — this is the "initial" the gate checks
// the run rises above.
let t=await u.currentEmbedding();p.initialCos=(0,c.cosine)(t,r),p.cos=p.initialCos,R=0,b(""),p.phase="run",p.running=!0,y()}catch(e){x(`text encode failed: ${e?.message??e}`)}finally{m.disabled=!1}}async function D(){if(!p.ready)return;p.running=!1,G=null,p.cos=null,p.initialCos=null,p.phase="reset",k+=1;let e=u;u=await (0,c.SplatOptimizer).create(i,o,s,{seed:k}),e.destroy(),P(),await u.renderImage(),g.value="",p.step=0,b(""),y()}async function F(){if(!p.ready||O)return;O=!0;let e=p.running;p.running=!1,p.phase="nudge",h.disabled=!0,k+=1,y();try{if(await u.nudge({seed:k}),await u.renderImage(),R=0,G){let e=await u.currentEmbedding();p.cos=(0,c.cosine)(e,G)}p.phase=e&&G?"run":"idle",p.running=e&&!!G,b(""),y()}catch(e){x(`nudge failed: ${e?.message??e}`)}finally{O=!1,h.disabled=!1}}// ── Boot ─────────────────────────────────────────────────────────────────────
...
i.export(t,"randomSplats",()=>g),i.export(t,"nudgeSplats",()=>m);var a=e("./raster"),o=e("./raster_wgsl"),s=e("../clip/vision"),u=e("./adam_wgsl");let n=12e3,d={scale:9,scaleJitter:.35,opacityRaw:.4,colorSpread:1.2},c={mean:1.5,logScale:.06,theta:.08,color:.12,opacity:.06},l=.18;class p{static async create(e,r,t,i={}){let[o,u,d]=r.inputShape;if(3!==o||256!==u||256!==d)throw Error(`optimize: CLIP inputShape [${o},${u},${d}] != [3,256,256] — the raster→CLIP copy assumes matching NCHW dims`);let c=i.G??n,l=i.cap??2048,f=await (0,a.RasterEngine).create(e,{H:256,W:256,G:c,cap:l,bg:i.bg??[.5,.5,.5]}),m=await (0,s.VisionTrainer).create(e,r,t);return f.setParams(i.initParams??g(c,i.seed??1,i.init)),f.zeroAdamState(),new p(e,f,m,i)}constructor(e,r,t,i){this.side=256,this.step_=0,this.device=e,this.raster=r,this.trainer=t,this.lrs=i.lrs??c,this.hyper=i.hyper??u.DEFAULT_HYPER,this.init=i.init}/** Target text embedding (raw, un-normalized — the −cos loss normalizes it).
   ...
   *  the optimizer, CLIP resources, prompt, step count, and Adam buffers alive. */async nudge(e={}){let r=this.raster.dims.G,t=await this.raster.readParams();m(t,r,e.seed??Date.now(),e.amount??l,e.init??this.init),this.raster.setParams(t)}/** Render the current splats without training; leaves the image on the GPU
   ...
   *  prove the loss decreases. */async currentEmbedding(){let e=this.device.createCommandEncoder();return this.raster.recordForward(e),e.copyBufferToBuffer(this.raster.image,0,this.trainer.inputBuffer,0,786432),this.trainer.encode(e,{backward:!1}),this.device.queue.submit([e.finish()]),v(this.device,this.trainer.outputBuffer,this.trainer.plan.embedDim)}destroy(){this.raster.destroy()}}function f(e,r){let t=0,i=0,a=0;for(let o=0;o<e.length;o++)t+=e[o]*r[o],i+=e[o]*e[o],a+=r[o]*r[o];return t/Math.sqrt(i*a||1)}function g(e,r=1,t={}){let i=t.scale??d.scale,a=t.scaleJitter??d.scaleJitter,s=t.opacityRaw??d.opacityRaw,u=t.colorSpread??d.colorSpread,n=r>>>0||1,c=()=>{let e=Math.imul((n=Math.imul(n,747796405)+2891336453>>>0)>>>(n>>>28)+4^n,277803737)>>>0;return(e=(e>>>22^e)>>>0)/4294967296},l=()=>{let e=0,r=0;for(;0===e;)e=c();for(;0===r;)r=c();return Math.sqrt(-2*Math.log(e))*Math.cos(2*Math.PI*r)},p=new Float32Array(e*o.PARAM_STRIDE),f=2*e,g=4*e,m=5*e,h=8*e,v=Math.log(i);for(let r=0;r<e;r++)p[0+2*r+0]=256*c(),p[0+2*r+1]=256*c(),p[f+2*r+0]=v+a*l(),p[f+2*r+1]=v+a*l(),p[g+r]=c()*Math.PI*2,p[m+3*r+0]=u*l(),p[m+3*r+1]=u*l(),p[m+3*r+2]=u*l(),p[h+r]=s;return p}function m(e,r,t=1,i=l,a={}){if(e.length!==r*o.PARAM_STRIDE)throw Error("nudgeSplats: wrong param length");let s=h(i,0,1);if(0===s)return e;let u=g(r,t,a),n=2*r,d=4*r,c=5*r,p=8*r,f=Math.log(.3),m=Math.log(64);for(let t=0;t<r;t++){var v,$,w,b,x,y,k,_;let r=0+2*t;e[r+0]=h((v=e[r+0],v+(u[r+0]-v)*s),0,256),e[r+1]=h(($=e[r+1],$+(u[r+1]-$)*s),0,256);let i=n+2*t;e[i+0]=h((w=e[i+0],w+(u[i+0]-w)*s),f,m),e[i+1]=h((b=e[i+1],b+(u[i+1]-b)*s),f,m);let a=d+t;e[a]=function(e,r,t){let i=2*Math.PI;return e+(((r-e+Math.PI)%i+i)%i-Math.PI)*t}(e[a],u[a],s);let o=c+3*t;e[o+0]=(x=e[o+0],x+(u[o+0]-x)*s),e[o+1]=(y=e[o+1],y+(u[o+1]-y)*s),e[o+2]=(k=e[o+2],k+(u[o+2]-k)*s);let l=p+t;e[l]=(_=e[l],_+(u[l]-_)*s)}return e}function h(e,r,t){return Math.max(r,Math.min(t,e))}// small readback helper (kept local — RasterEngine's is private, and the CLIP
...
function f(e,r){if(!e)throw Error(`raster_wgsl: ${r}`)}/** WGSL f32 literal — always has a '.' or exponent so it is not parsed as int. */function g(e){f(Number.isFinite(e),`non-finite literal ${e}`);let r=e.toString();return/[.eE]/.test(r)||(r+=".0"),r}let m=e=>`${e>>>0}u`;function h(e){f(e.H>0&&e.W>0&&e.G>0,"H,W,G must be positive"),f(e.H%a==0&&e.W%a==0,`H,W must be multiples of ${a}`),f((e.cap&e.cap-1)==0&&e.cap>0,"cap must be a power of two"),f(4*e.cap<=16384,`cap*4 (${4*e.cap}B) exceeds 16KB workgroup storage`);let r=e.W/a,t=e.H/a;return{H:e.H,W:e.W,G:e.G,cap:e.cap,tilesX:r,tilesY:t,numTiles:r*t,bg:e.bg??[.5,.5,.5],gradScale:e.gradScale??65536}}// ---------------------------------------------------------------------------
// Shared WGSL fragments (inlined per kernel — each module stays standalone)
// ---------------------------------------------------------------------------
/** Segment base offsets into the SoA params/gradRaw/m/v buffers. */function v(e){return{mean:0,logScale:2*e.G,theta:4*e.G,colorRaw:5*e.G,opacityRaw:8*e.G}}function $(e){let r=h(e),t=v(r);return/* wgsl */`
...
fn main(@builtin(global_invocation_id) gid : vec3u) {
  let g = gid.x;
  if (g >= ${m(r.G)}) { return; }
  let mx  = params[${m(t.mean)} + g * 2u + 0u];
  let my  = params[${m(t.mean)} + g * 2u + 1u];
  let lsx = params[${m(t.logScale)} + g * 2u + 0u];
  let lsy = params[${m(t.logScale)} + g * 2u + 1u];
  let th  = params[${m(t.theta)} + g];
  let cr0 = params[${m(t.colorRaw)} + g * 3u + 0u];
  let cr1 = params[${m(t.colorRaw)} + g * 3u + 1u];
  let cr2 = params[${m(t.colorRaw)} + g * 3u + 2u];
  let opr = params[${m(t.opacityRaw)} + g];
...
`}function w(e){let r=h(e);return/* wgsl */`
  ...
  for (var ty = ty0; ty <= ty1; ty = ty + 1) {
    ...
    for (var tx = tx0; tx <= tx1; tx = tx + 1) {
      ...
      if (ellipse_hit(mx, my, a, b, c, tau, rx0, ry0, rx1, ry1)) {
        ...
        if (slot < ${m(r.cap)}) { binnedIds[tile * ${m(r.cap)} + slot] = g; }
      }
    }
  }
}
`}function b(e){let r=h(e),t=r.H*r.W;return/* wgsl */`
...
fn main(@builtin(workgroup_id) wg : vec3u,
  ...
  let tileId = wg.x;
  if (tileId >= ${m(r.numTiles)}) { return; }
  let count = min(tileCounts[tileId], ${m(r.cap)});
  let start = tileId * ${m(r.cap)};
  ...
  let tileX = tileId % ${m(r.tilesX)};
  let tileY = tileId / ${m(r.tilesX)};
  ...
  if (x < ${m(r.W)} && y < ${m(r.H)}) {
    ...
    let pix = y * ${m(r.W)} + x;
    image[0u * ${m(t)} + pix] = accR + T * ${g(r.bg[0])};
    image[1u * ${m(t)} + pix] = accG + T * ${g(r.bg[1])};
    image[2u * ${m(t)} + pix] = accB + T * ${g(r.bg[2])};
  }
  atomicMax(&sh_maxstop, localStop);
  workgroupBarrier();
  if (tid == 0u) { tileStop[tileId] = atomicLoad(&sh_maxstop); }
}
`}function x(e){let r=h(e),t=r.H*r.W,i=g(r.gradScale);return/* wgsl */`
...
fn main(@builtin(workgroup_id) wg : vec3u,
  ...
  let pyc = f32(y) + 0.5;
  let pix = y * ${m(r.W)} + x;
  let goR = gradImage[0u * ${m(t)} + pix];
  let goG = gradImage[1u * ${m(t)} + pix];
  let goB = gradImage[2u * ${m(t)} + pix];
...
`}function y(e){let r=h(e),t=v(r),i=g(1/r.gradScale);return/* wgsl */`
...
fn main(@builtin(global_invocation_id) gid : vec3u) {
  ...
  gradRaw[${m(t.mean)} + g * 2u + 0u] = gmx;
  gradRaw[${m(t.mean)} + g * 2u + 1u] = gmy;
  gradRaw[${m(t.logScale)} + g * 2u + 0u] = glsx;
  gradRaw[${m(t.logScale)} + g * 2u + 1u] = glsy;
  gradRaw[${m(t.theta)} + g] = gth;
  gradRaw[${m(t.colorRaw)} + g * 3u + 0u] = gc0 * col0 * (1.0 - col0);
  gradRaw[${m(t.colorRaw)} + g * 3u + 1u] = gc1 * col1 * (1.0 - col1);
  gradRaw[${m(t.colorRaw)} + g * 3u + 2u] = gc2 * col2 * (1.0 - col2);
  gradRaw[${m(t.opacityRaw)} + g] = gop * opv * (1.0 - opv);
}
...
${g.join("\n")}
  ...
  dst[out + 2u] = ${f("acc.z")};
  dst[out + 3u] = ${f("acc.w")};
}`;return{label:`conv${e.k} ${e.cin}->${e.cout} g${e.groups} @${e.outH}x${e.outW}`,code:m,workgroups:[Math.ceil(t/64),e.cout,1],buffers:[{kind:"weights"},{kind:"slot",slot:e.src},{kind:"slot",slot:e.dst}]}}(e)];// depthwise = spatial, cpg=1
