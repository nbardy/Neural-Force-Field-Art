...
import { labConversionFunctions, skewL } from "./color";
...
const colorSet = [magnetaSet, tealSet, aquamarineSet];

export function drawAgents(
  canvas,
  agents: AgentBatch,
  config: { predictField?: boolean; model: any }
) {
  ...
  const { predictField, model } = config;
  ...
  if (predictField) {
    ...
    drawTriangles(canvas, tiles, forceUpdates, forceUpdatesMagnitude);
  }

  // Draw Agents
  agents.agentPositions.forEach((_, i) => {
    const posColor = colorSet[i][0];
    const velColor = colorSet[i][1];
