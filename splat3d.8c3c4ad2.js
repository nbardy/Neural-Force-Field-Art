...
var l=d(t);"object"==typeof exports&&"undefined"!=typeof module?module.exports=l:"function"==typeof define&&define.amd?define(function(){return l}):a&&(this[a]=l)}}({e5WXe:[function(e,r,t){let i,a,o,s,u,n;/// <reference types="@webgpu/types" />
var d=e("./splat3d/cameras"),c=e("./splat3d/optimize"),l=e("./splat/fetch_progress");let p={gpu:!!navigator.gpu,ready:!1,running:!1,step:0,view:0,cos:null,initialCos:null,error:null,phase:"boot",promptMode:"camera",blackBgText:!0};window.__splat3d=p;let f=document.getElementById("grid"),g=document.getElementById("prompt"),h=document.getElementById("view"),m=document.getElementById("promptMode"),v=document.getElementById("bgTextMode"),w=document.getElementById("optimize"),$=document.getElementById("reset"),b=document.getElementById("readout"),x=document.getElementById("notice");function y(e){x.textContent=e}function k(e){p.error=e,p.phase="error",y(e),b.textContent="—",console.error("[splat3d_page]",e)}function _(){p.step=s?s.stepCount:0;let e=s?.cameras[P]?.name??"view",r=[`step ${p.step}`,e];if(r.push("camera"===p.promptMode?"camera text":"same text"),p.blackBgText&&r.push("black bg"),null!==p.cos){let e=p.initialCos??p.cos,t=p.cos-e;r.push(`cos ${p.cos.toFixed(4)}`),r.push(`init ${e.toFixed(4)}`),r.push(`Δ ${t>=0?"+":""}${t.toFixed(4)}`)}p.phase&&"run"!==p.phase&&r.push(`(${p.phase})`),b.textContent=r.join("  \xb7  ")}let B=1,P=0,C=!1,S=null,j=[],R=[],E=/* wgsl */`
@vertex
...
`;async function D(){i.pushErrorScope("validation");let e=i.createShaderModule({code:E});u=i.createRenderPipeline({layout:"auto",vertex:{module:e,entryPoint:"vs"},fragment:{module:e,entryPoint:"fs",targets:[{format:n}]},primitive:{topology:"triangle-list"}});let r=await i.popErrorScope();if(r)throw Error(`blit pipeline invalid: ${r.message}`)}function T(){S=i.createBindGroup({layout:u.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:s.raster.image}}]})}let A=Function("u","return import(u)"),G=null,O=null;async function I(){if(O)return;let e=await A("https://cdn.jsdelivr.net/npm/@huggingface/transformers@4.2.0/+esm");e.env.allowRemoteModels=!0;let r="Xenova/mobileclip_s0";G=await e.AutoTokenizer.from_pretrained(r),O=await e.CLIPTextModelWithProjection.from_pretrained(r,{dtype:"fp32",device:"wasm"})}async function q(e){await I();let r=await G(e,{padding:"max_length",max_length:77,truncation:!0}),t=await O(r),i=t.text_embeds.data,a=new Float32Array(512);for(let e=0;e<512;e++)a[e]=i[e];return a}let M=null,W=0,F=!1;async function L(){if(M&&!F){F=!0;try{let e=await s.currentEmbedding(P),r=(0,c.cosine)(e,M[P]);p.cos=r,null===p.initialCos&&(p.initialCos=r),_()}finally{F=!1}}}function z(){p.running&&M&&(s.step(P),C=!0,W+=1,p.step=s.stepCount,W>=3&&(W=0,L())),C&&(function(){if(!S||!j.length)return;let e=i.createCommandEncoder();for(let r=0;r<j.length;r++)s.raster.recordForward(e,r),function(e,r){if(!S)return;let t=e.beginRenderPass({colorAttachments:[{view:r.getCurrentTexture().createView(),loadOp:"clear",storeOp:"store",clearValue:{r:0,g:0,b:0,a:1}}]});t.setPipeline(u),t.setBindGroup(0,S),t.draw(3),t.end()}(e,j[r]);i.queue.submit([e.finish()])}(),C=!1),requestAnimationFrame(z)}async function Y(){if(!p.ready)return;let e=g.value.trim()||"a photo of a cat";w.disabled=!0,$.disabled=!0,h.disabled=!0,m.disabled=!0,v.disabled=!0,p.running=!1,p.phase="encoding",p.cos=null,p.initialCos=null,p.promptMode="same"===m.value?"same":"camera",p.blackBgText="none"!==v.value,_();try{let r=[];for(let t=0;t<s.cameras.length;t++){y(`encoding prompt ${t+1}/${s.cameras.length}…`);let i="camera"===p.promptMode?(0,d.buildViewPrompt)(e,s.cameras[t],p.blackBgText):(0,d.buildBasePrompt)(e,p.blackBgText);r.push(await q(i))}M=r,s.setViewPrompts(r);let t=await s.currentEmbedding(P);p.initialCos=(0,c.cosine)(t,r[P]),p.cos=p.initialCos,W=0,y(""),p.phase="run",p.running=!0,C=!0,_()}catch(e){k(`text encode failed: ${e?.message??e}`)}finally{w.disabled=!1,$.disabled=!1,h.disabled=!1,m.disabled=!1,v.disabled=!1}}async function H(){if(!p.ready)return;p.running=!1,M=null,p.cos=null,p.initialCos=null,p.phase="reset",B+=1;let e=s;s=await (0,c.Splat3DOptimizer).create(i,a,o,{seed:B}),e.destroy(),T(),C=!0,p.step=0,y(""),_()}async function U(){X(Math.max(0,h.selectedIndex)),p.ready&&(p.cos=null,p.initialCos=null,M&&L(),_())}function X(e){P=Math.max(0,Math.min(s?s.cameras.length-1:0,0|e)),p.view=P,h.selectedIndex=P;for(let e=0;e<R.length;e++)R[e].classList.toggle("active",e===P)}function V(){p.promptMode="same"===m.value?"same":"camera",p.blackBgText="none"!==v.value,M&&(p.running=!1,M=null,p.cos=null,p.initialCos=null,p.phase="idle",y("")),_()}async function N(){let e;if(!navigator.gpu){k("this page needs WebGPU (no navigator.gpu) — use Chrome/Edge with WebGPU enabled."),w.disabled=!0,$.disabled=!0,h.disabled=!0,m.disabled=!0,v.disabled=!0;return}p.phase="adapter";let r=await navigator.gpu.requestAdapter();if(!r)return k("no WebGPU adapter available.");i=await r.requestDevice(),i.addEventListener?.("uncapturederror",e=>{console.error("[webgpu]",e.error?.message??e.error)}),n=navigator.gpu.getPreferredCanvasFormat(),p.phase="weights";// Prod (GitHub Pages) fetches the packed weights from the HF Hub: GitHub
...
let t=["localhost","127.0.0.1"].includes(location.hostname),u=t?"/models/mobileclip_s0/":"https://huggingface.co/Nbardy/nff-clip-splat-weights/resolve/main/",d=t?"":" from HF";b.textContent=`fetching CLIP plan${d}…`;let g=await fetch(u+"plan_train.json");if(!g.ok)return k(`plan_train.json fetch ${g.status} from ${u}`);a=await g.json();try{e=await (0,l.fetchArrayBufferWithProgress)(u+"weights_train.bin",e=>{b.textContent=(0,l.formatProgress)(`loading CLIP weights${d}`,e)})}catch(e){return k(`weights_train.bin fetch failed from ${u}: ${e?.message??e}`)}o=new Float32Array(e),p.phase="optimizer",b.textContent="building 3D optimizer…",await D(),s=await (0,c.Splat3DOptimizer).create(i,a,o,{seed:B}),function(){h.textContent="",f.textContent="",j=[],R=[];for(let e=0;e<s.cameras.length;e++){let r=s.cameras[e],t=document.createElement("option");t.value=r.name,t.textContent=r.name,h.appendChild(t);let a=document.createElement("div");a.className="tile";let o=document.createElement("canvas");o.className="view",o.width=256,o.height=256;let u=document.createElement("div");u.className="label",u.textContent=r.name,a.append(o,u),a.addEventListener("click",()=>{X(e),p.cos=null,p.initialCos=null,M&&L(),_()});let d=o.getContext("webgpu");d.configure({device:i,format:n,alphaMode:"opaque"}),f.appendChild(a),j.push(d),R.push(a)}X(P)}(),T(),C=!0,p.ready=!0,p.phase="idle",w.disabled=!1,$.disabled=!1,h.disabled=!1,m.disabled=!1,v.disabled=!1,y(""),_(),requestAnimationFrame(z)}w.addEventListener("click",()=>void Y()),$.addEventListener("click",()=>void H()),h.addEventListener("change",()=>void U()),m.addEventListener("change",V),v.addEventListener("change",V),g.addEventListener("keydown",e=>{"Enter"===e.key&&Y()}),N().catch(e=>k(`boot failed: ${e?.message??e}`))},{"./splat3d/cameras":"3Dl1Z","./splat3d/optimize":"dTqrt","./splat/fetch_progress":"8QffC"}],"3Dl1Z":[function(e,r,t){var i=e("@parcel/transformer-js/src/esmodule-helpers.js");i.defineInteropFlag(t),i.export(t,"DEFAULT_3D_CAMERAS",()=>a),i.export(t,"BLACK_BACKGROUND_PROMPT",()=>o),i.export(t,"buildBasePrompt",()=>s),i.export(t,"buildViewPrompt",()=>u),i.export(t,"prepareCamera",()=>n);let a=[{name:"top",promptSuffix:"a top-down camera angle",eye:[0,3.3,0],target:[0,0,0],up:[0,0,-1]},{name:"front",promptSuffix:"a front-facing camera angle",eye:[0,0,3],target:[0,0,0]},{name:"right",promptSuffix:"a camera angle from the right side",eye:[3,0,0],target:[0,0,0]},{name:"back",promptSuffix:"a camera angle from behind",eye:[0,0,-3],target:[0,0,0]},{name:"left",promptSuffix:"a camera angle from the left side",eye:[-3,0,0],target:[0,0,0]},{name:"front-left-high",promptSuffix:"an elevated 45 degree camera angle from the front left looking down",eye:[-2.16,1.7,2.16],target:[0,0,0]},{name:"front-right-high",promptSuffix:"an elevated 45 degree camera angle from the front right looking down",eye:[2.16,1.7,2.16],target:[0,0,0]},{name:"back-right-low",promptSuffix:"a low 45 degree camera angle from the rear right looking up",eye:[2.16,-1.3,-2.16],target:[0,0,0]},{name:"back-left-low",promptSuffix:"a low 45 degree camera angle from the rear left looking up",eye:[-2.16,-1.3,-2.16],target:[0,0,0]}],o="on a black background";function s(e,r=!0){let t=e.trim()||"a photo of a cat";return!r||/\bblack background\b/i.test(t)?t:`${t}, ${o}`}function u(e,r,t=!0){return s(`${e.trim()||"a photo of a cat"}, ${r.promptSuffix}`,t)}function n(e,r){var t,i;let a=l((t=e.target,i=e.eye,[t[0]-i[0],t[1]-i[1],t[2]-i[2]])),o=e.up??[0,1,0],s=l(d(a,o));1e-5>c(s)&&(s=l(d(a,[0,0,1])));let u=l(d(s,a)),n=(e.fovYDeg??50)*Math.PI/180;return{...e,right:s,cameraUp:u,forward:a,focalPx:.5*r/Math.tan(.5*n)}}function d(e,r){return[e[1]*r[2]-e[2]*r[1],e[2]*r[0]-e[0]*r[2],e[0]*r[1]-e[1]*r[0]]}function c(e){return Math.hypot(e[0],e[1],e[2])}function l(e){let r=1/Math.max(c(e),1e-9);return[e[0]*r,e[1]*r,e[2]*r]}},{"@parcel/transformer-js/src/esmodule-helpers.js":"k3151"}],k3151:[function(e,r,t){t.interopDefault=function(e){return e&&e.__esModule?e:{default:e}},t.defineInteropFlag=function(e){Object.defineProperty(e,"__esModule",{value:!0})},t.exportAll=function(e,r){return Object.keys(e).forEach(function(t){"default"===t||"__esModule"===t||r.hasOwnProperty(t)||Object.defineProperty(r,t,{enumerable:!0,get:function(){return e[t]}})}),r},t.export=function(e,r,t){Object.defineProperty(e,r,{enumerable:!0,get:t})}},{}],dTqrt:[function(e,r,t){/// <reference types="@webgpu/types" />
var i=e("@parcel/transformer-js/src/esmodule-helpers.js");i.defineInteropFlag(t),i.export(t,"LEGIBLE_3D_G",()=>d),i.export(t,"LEGIBLE_3D_INIT",()=>c),i.export(t,"Splat3DOptimizer",()=>l),i.export(t,"randomSplats3D",()=>p),i.export(t,"cosine",()=>f);var a=e("../clip/vision"),o=e("../splat/adam_wgsl"),s=e("./cameras"),u=e("./raster"),n=e("./raster_wgsl");let d=4096,c={radius:.075,radiusJitter:.35,opacityRaw:.3,colorSpread:1.2,positionSpread:.9};class l{static async create(e,r,t,i={}){let[o,n,c]=r.inputShape;if(3!==o||256!==n||256!==c)throw Error(`splat3d: CLIP inputShape [${o},${n},${c}] != [3,256,256]`);let f=(i.cameras??(0,s.DEFAULT_3D_CAMERAS)).map(e=>(0,s.prepareCamera)(e,256)),g=i.G??d,h=await (0,u.Raster3DEngine).create(e,{H:256,W:256,G:g,cap:i.cap??2048,bg:i.bg??[0,0,0],cameras:f}),m=await (0,a.VisionTrainer).create(e,r,t);return h.setParams(i.initParams??p(g,i.seed??1,i.init)),h.zeroAdamState(),new l(e,h,m,f,i)}constructor(e,r,t,i,a){this.side=256,this.step_=0,this.hasPrompts=!1,this.device=e,this.raster=r,this.trainer=t,this.cameras=i,this.lrs=a.lrs??u.DEFAULT_3D_LRS,this.hyper=a.hyper??o.DEFAULT_HYPER,this.textBuffers=i.map((r,i)=>e.createBuffer({label:`splat3d-text-${i}`,size:4*t.plan.textDim,usage:12}))}setViewPrompts(e){if(e.length!==this.cameras.length)throw Error(`splat3d: ${e.length} text embeds for ${this.cameras.length} cameras`);for(let r=0;r<e.length;r++){if(e[r].length!==this.trainer.plan.textDim)throw Error(`splat3d: view ${r} text ${e[r].length} != ${this.trainer.plan.textDim}`);this.device.queue.writeBuffer(this.textBuffers[r],0,e[r])}this.hasPrompts=!0}step(e=0){if(!this.hasPrompts)throw Error("splat3d: setViewPrompts() before step()");let r=this.device.createCommandEncoder();this.raster.recordClearRawGrad(r);for(let e=0;e<this.cameras.length;e++)r.copyBufferToBuffer(this.textBuffers[e],0,this.trainer.textBuffer,0,4*this.trainer.plan.textDim),this.raster.recordForward(r,e),r.copyBufferToBuffer(this.raster.image,0,this.trainer.inputBuffer,0,786432),this.trainer.encode(r,{backward:!0}),r.copyBufferToBuffer(this.trainer.inputGradBuffer,0,this.raster.gradImage,0,786432),this.raster.recordBackwardAdd(r,e);this.step_+=1,this.raster.recordAdam(r,this.step_,this.lrs,this.hyper),this.raster.recordForward(r,e),this.device.queue.submit([r.finish()])}get stepCount(){return this.step_}async renderView(e=0){return this.raster.runForward(e),this.raster.readImage()}renderViewToImage(e=0){this.raster.runForward(e)}async currentEmbedding(e=0){let r=this.device.createCommandEncoder();return this.raster.recordForward(r,e),r.copyBufferToBuffer(this.raster.image,0,this.trainer.inputBuffer,0,786432),this.trainer.encode(r,{backward:!1}),this.device.queue.submit([r.finish()]),g(this.device,this.trainer.outputBuffer,this.trainer.plan.embedDim)}destroy(){for(let e of(this.raster.destroy(),this.textBuffers))try{e.destroy()}catch(e){}}}function p(e,r=1,t={}){let i=t.radius??c.radius,a=t.radiusJitter??c.radiusJitter,o=t.opacityRaw??c.opacityRaw,s=t.colorSpread??c.colorSpread,u=t.positionSpread??c.positionSpread,d=r>>>0||1,l=()=>{let e=Math.imul((d=Math.imul(d,747796405)+2891336453>>>0)>>>(d>>>28)+4^d,277803737)>>>0;return(e=(e>>>22^e)>>>0)/4294967296},p=()=>{let e=0,r=0;for(;0===e;)e=l();for(;0===r;)r=l();return Math.sqrt(-2*Math.log(e))*Math.cos(2*Math.PI*r)},f=new Float32Array(e*n.PARAM_STRIDE_3D),g=3*e,h=4*e,m=7*e,v=Math.log(i);for(let r=0;r<e;r++)f[0+3*r+0]=(2*l()-1)*u,f[0+3*r+1]=(2*l()-1)*u,f[0+3*r+2]=(2*l()-1)*u,f[g+r]=v+a*p(),f[h+3*r+0]=s*p(),f[h+3*r+1]=s*p(),f[h+3*r+2]=s*p(),f[m+r]=o;return f}function f(e,r){let t=0,i=0,a=0;for(let o=0;o<e.length;o++)t+=e[o]*r[o],i+=e[o]*e[o],a+=r[o]*r[o];return t/Math.sqrt(i*a||1)}async function g(e,r,t){let i=e.createBuffer({size:4*t,usage:9}),a=e.createCommandEncoder();a.copyBufferToBuffer(r,0,i,0,4*t),e.queue.submit([a.finish()]),await i.mapAsync(1);let o=new Float32Array(i.getMappedRange().slice(0));return i.unmap(),i.destroy(),o}},{"../clip/vision":"lNzsi","../splat/adam_wgsl":"kfWkJ","./cameras":"3Dl1Z","./raster":"AoYYi","./raster_wgsl":"fuyeU","@parcel/transformer-js/src/esmodule-helpers.js":"k3151"}],lNzsi:[function(e,r,t){/**
...
}`;return{label:`attn_core_bwd h${t} n${o}`,code:u,workgroups:[t,1,1],buffers:[s(e.savedQkv),s(e.dY),s(e.dX)]}}(e))}}function n(e){return e.backward.map(u)}},{"./vision_wgsl":"oFDUc","@parcel/transformer-js/src/esmodule-helpers.js":"k3151"}],kfWkJ:[function(e,r,t){/**
...
fn main(@builtin(global_invocation_id) gid : vec3u) {
  ...
  let radiusWorld = clamp(exp(params[${h(i.logRadius)} + g]), ${g(l)}, ${g(p)});
  let radiusPx = max(FOCAL_PX * radiusWorld / safeZ, 0.25);
  let invR2 = 1.0 / max(radiusPx * radiusPx, ${g(c)});
...
fn main(@builtin(global_invocation_id) gid : vec3u) {
  ...
  if (op <= ${g(u)}) { return; }
  let ratio = max(${g(u)} / max(op, ${g(c)}), ${g(c)});
  let tau = -2.0 * log(ratio);
  if (!(tau > 0.0)) { return; }

  let sx = derived[base + 0u];
  let sy = derived[base + 1u];
  let invR2 = max(derived[base + 2u], ${g(c)});
...
fn main(@builtin(workgroup_id) wg : vec3u,
  ...
  if (x < ${h(r.W)} && y < ${h(r.H)}) {
    ...
    for (var i = 0u; i < count; i = i + 1u) {
      ...
      let alpha = min(${g(n)}, raw);
      if (alpha < ${g(u)}) { continue; }
      let w = T * alpha;
...
fn main(@builtin(workgroup_id) wg : vec3u,
  ...
  for (var i = 0u; i < stopc; i = i + 1u) {
    ...
    if (power > 0.0) { continue; }
    let alpha = min(${g(n)}, derived[b + 10u] * exp(power));
    if (alpha < ${g(u)}) { continue; }
    T = T * (1.0 - alpha);
  ...
  for (var ii = i32(endi) - 1; ii >= 0; ii = ii - 1) {
    ...
    let gate = select(0.0, 1.0, raw < ${g(n)});
    ...
    fixadd(b, 10u, gRaw * (raw / max(op, ${g(c)})));
...
fn main(@builtin(global_invocation_id) gid : vec3u) {
  ...
  let er = exp(lr);
  let gateR = select(0.0, 1.0, er > ${g(l)} && er < ${g(p)});
...
`}function P(e){return[{name:"position",offset:0,length:3*e},{name:"logRadius",offset:3*e,length:e},{name:"color",offset:4*e,length:3*e},{name:"opacity",offset:7*e,length:e}]}},{"@parcel/transformer-js/src/esmodule-helpers.js":"k3151"}],"8QffC":[function(e,r,t){/**
